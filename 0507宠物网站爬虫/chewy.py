...
def analyzeSoup(soup,info_tmp):
    one_page_list = []
    article_list = soup.select('article.product-holder')
    for article in article_list:
        info = info_tmp.copy()
        desc_tag = article.select('.content > h2')[0]
        price_tag = article.select('.price')[0]
        price_old_tag = article.select('.price-old')[0]
        href_tag = article.select('a')[0]
        ...
        price_old_arr = price_pattern.findall(price_old_tag.text)
        if len(price_old_arr) > 0:
            info['old_price'] = price_old_arr[0]
        else:
            info['old_price'] = info['price']
        info['price'] = float(info['price'])
        info['old_price'] = float(info['old_price'])
        info['url'] = 'https://' + info_tmp['brand'] + href_tag.attrs['href']
        print(info)
...
def getFirsrPageUrl(soup,info_tmp):
    first_page_url = ''
    first_page_tag_list = soup.select('.cw-pagination--is-active')
    if len(first_page_tag_list) > 0:
        first_page_tag = first_page_tag_list[0]
        first_page_url = 'https://' + info_tmp['brand'] + first_page_tag.attrs['href']
...
def getTotalPageNum(url,info_tmp):
    ...
    total_tag_list = soup.select('.results-count')
    if len(total_tag_list) > 0:
        total_tag = total_tag_list[0]
        print(total_tag.text)
        total_pattern = re.compile(r'of ([0-9]+) result')
        ...
        first_page_url = getFirsrPageUrl(soup,info_tmp)

        return page_num,first_page_url,one_page_list
    else:
        return 0,'',[]
...
def getOneCatePage(url,info_tmp,info_flied_arr,tmp_data_dir,record_path):
    ...
    if url not in done_url:
        total_page_num,first_page_url,first_page_list = getTotalPageNum(url,info_tmp)
        if total_page_num == 0:
            pass
            ...
            for i in range(1,total_page_num+1):
                page_url = '{0}&page={1}'.format(first_page_url,i)
...
def getCateList(url_excel_path):
    cate_list = []
    url_df = pd.read_excel(url_excel_path,usecols=[0,1,2])
...
if __name__ == '__main__':
    ...
    cate_list_1 = [{'cate_name': 'Dog Sweaters & Hoodies', 'cate_remark': '毛衣卫衣', 'cate_url': 'https://www.chewy.com/b/sweaters-1962'},
                   {'cate_name': 'Dog Coats & Jackets', 'cate_remark': '外套', 'cate_url': 'https://www.chewy.com/b/jackets-1903'}]
